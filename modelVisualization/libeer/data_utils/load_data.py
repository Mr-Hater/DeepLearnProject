...
from LibEER.data_utils.preprocess import preprocess, label_process
...
def get_data(setting=None):
    ...
    # 获取统一格式的数据，加载数据集并整合为(session, subject, trail)格式
    ...
    all_data, feature_dim = preprocess(
        ...
        pass_band=setting.pass_band,  # 通带频率范围
        extract_bands=setting.extract_bands,  # 要提取的频带
        sample_length=setting.sample_length,  # 样本长度
        stride=setting.stride,  # 滑动步长
        time_window=setting.time_window,  # 时间窗口
        overlap=setting.overlap,  # 重叠率
        # 是否仅进行分段处理，对于需要特征提取的数据集强制设为True
        only_seg=setting.only_seg if setting.dataset not in extract_dataset else True,
        feature_type=setting.feature_type,  # 特征类型
        eog_clean=setting.eog_clean  # 眼电伪迹去除
    )

    # 对标签进行处理
    all_data, all_label, num_classes = label_process(
        data=all_data,  # 预处理后的数据
        label=label,  # 原始标签
        bounds=setting.bounds,  # 标签边界
        onehot=setting.onehot,  # 是否使用one-hot编码
        label_used=setting.label_used  # 使用的标签类型
    )
...
def read_seed_raw(dir_path):
    """
        ...
        输入: 45个文件(3个session, 15个受试者)，每个文件包含15个trials，采样率200Hz
        输出: 以trial为基本单位的EEG信号，保持原始数据集的采样率
        输出形状: (session, subject, trail, channel, raw_data), (session, subject, trail, label)
    """

    # 设置预处理后的EEG数据路径
    dir_path += "/Preprocessed_EEG"
    # 定义三个session中每个受试者对应的数据文件名
    ...
    # 从label.mat文件中提取所有trials的标签
    label = np.array(loadmat(f"{dir_path}/label.mat")['label'])
    # 将标签扩展到三个session，每个session15个受试者，每个受试者15个trials
    labels = np.tile(label[0] + 1, (3, 15, 1))

    # 创建空列表来存储EEG数据，形状为(3个session, 15个受试者, 15个trials)
